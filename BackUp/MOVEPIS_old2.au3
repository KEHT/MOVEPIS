...
Func ProcCMDfiles($aFiles)
	...
	For $i = 0 To UBound($aFiles) - 1
		...
		if FileExists($aFiles[$i]) Then
			if FileMove($aFiles[$i], $cOutputFolder, $FC_OVERWRITE  + $FC_CREATEPATH) == 0 Then
		...
		ElseIf FileExists($cInputFolder & "\" & $aFiles[$i]) Then
			if FileMove($cInputFolder & "\" & $aFiles[$i], $cOutputFolder, $FC_OVERWRITE  + $FC_CREATEPATH) == 0 Then
		...
		ElseIf FileExists("\\alpha3\E\FR\OC" & "\" & $aFiles[$i]) Then
			if FileMove($cInputFolderDefault & "\" & $aFiles[$i], $cOutputFolder, $FC_OVERWRITE  + $FC_CREATEPATH) Then
