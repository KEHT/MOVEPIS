;~ 06/30/2014 - sjohnson@gpo.gov - Alpha version (0.90) MOVEPIS to process CMD files
...
Global $cInputFolderDefault = "\\alpha3\E\FR\FM"
...
GUICreate("MOVEPIS + COMBINE v0.9", 350, 300)
...
$Process_Button = GUICtrlCreateButton("PROCESS FILE", 100, 205, 150, 45, $BS_CENTER + $BS_DEFPUSHBUTTON)
GUICtrlSetState($Process_Button, $GUI_DISABLE)
Global $progressbar = GUICtrlCreateProgress(20, 275, 310, 10, $PBS_SMOOTH)
...
While 1
	...
	Switch $msg
		...
		Case $Process_Button
			...
			ProcCMDfiles($aFiles, ProcSGMfile(GUICtrlRead($inputFile)))
	EndSwitch
...
Func ProcCMDfiles($aFiles, $aCMDfilename)
	...
	Dim $progpercent = 10
	Dim $progincrement = Round((UBound($aFiles) - 1) / $progpercent)
...
;~ 			$cInputFileText = FileRead($aFiles[$i])
		...
		$cInputFileText = StringRegExpReplace($cInputFileText, '&thnsp; ', '&thnsp;')
		$cInputFileText = StringRegExpReplace($cInputFileText, ChrW(26), '')
...
;~ 		Search for bell codes
		...
		Else
			...
			FileClose($hFile)
		EndIf
		GUICtrlSetData($progressbar, (100 - ($progincrement * (UBound($aFiles) - 1))))
	Next
	...
	Else
		...
		GUICtrlSetData($progressbar, 0)
		Exit MsgBox($MB_ICONINFORMATION, "SUCCESS!", $aCMDfilename[0] & " has been combined to " & $sSGMfilename & @CRLF & "MOVEPIS Done . . .")
	EndIf
